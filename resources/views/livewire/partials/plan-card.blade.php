...
@if($isCurrent)
<div class="coin-plan-card" style="position: relative; padding: 24px; border-radius: 18px; {{ $currentCardStyle }}">
  <div style="position: absolute; top: -10px; left: 24px; padding: 4px 10px; border-radius: 7px; background: linear-gradient(140deg, oklch(0.88 0.12 192), oklch(0.66 0.13 205)); font-family: 'JetBrains Mono', monospace; font-size: 9px; letter-spacing: 0.12em; color: #04121f;">{{ mb_strtoupper(__('coin.invest.current_badge')) }}</div>
  <div style="display: flex; align-items: center; justify-content: space-between;">
    ...
    <span style="font-family: 'JetBrains Mono', monospace; font-size: 10px; letter-spacing: 0.12em; color: rgba(214,238,248,0.72);">{{ $plan->displayTierLabel() }}</span>
  </div>
  ...
  <div style="display: flex; flex-direction: column; gap: 13px; font-size: 13px; flex: 1;">
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.min_investment') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedMinDeposit() ?? $plan->formattedComputeLabel() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.term') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedDuration() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.annual_return') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedAnnualProfit() ?? ($plan->formattedDailyEstimate() ?? __('coin.invest.estimated')) }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.infrastructure') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->displayInfra() }}</span></div>
  </div>
  ...
  <button type="button" wire:click="setSection(2)" class="coin-plan-card__cta" style="padding: 11px; border-radius: 10px; border: 1px solid {{ $currentMuted ? 'rgba(150,235,250,0.2)' : 'oklch(0.86 0.11 195 / 0.5)' }}; background: {{ $currentMuted ? 'rgba(150,235,250,0.06)' : 'linear-gradient(140deg, oklch(0.86 0.12 192), oklch(0.66 0.13 205))' }}; color: {{ $currentMuted ? '#e6f4fa' : '#04121f' }}; font-family: inherit; font-size: 13.5px; font-weight: {{ $currentMuted ? '500' : '600' }}; cursor: pointer;">{{ __('coin.invest.manage') }}</button>
</div>
@elseif($isEnterprise)
<div class="coin-plan-card" style="position: relative; padding: 24px; border-radius: 18px; {{ $selectedStyle ?: 'border: 1px solid rgba(180,180,255,0.16); background: linear-gradient(170deg, rgba(120,110,220,0.13), rgba(150,235,250,0.02));' }}">
  @if($isSelected)
  <div style="position: absolute; top: -10px; left: 24px; padding: 4px 10px; border-radius: 7px; background: linear-gradient(140deg, oklch(0.88 0.12 192), oklch(0.66 0.13 205)); font-family: 'JetBrains Mono', monospace; font-size: 9px; letter-spacing: 0.12em; color: #04121f;">{{ mb_strtoupper(__('coin.invest.selected')) }}</div>
  @endif
  <div style="display: flex; align-items: center; justify-content: space-between;">
  ...
  <div style="display: flex; flex-direction: column; gap: 13px; font-size: 13px; flex: 1;">
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.min_investment') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedMinDeposit() ?? $plan->formattedComputeLabel() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.term') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedDuration() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.annual_return') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedAnnualProfit() ?? __('coin.invest.estimated') }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.infrastructure') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->displayInfra() }}</span></div>
  </div>
  ...
  <button type="button" wire:click="selectPlan({{ $plan->id }})" wire:loading.attr="disabled" wire:target="{{ $selectTarget }}" class="coin-plan-card__cta" style="padding: 11px; border-radius: 10px; border: 1px solid rgba(180,180,255,0.28); background: rgba(150,140,255,0.1); color: #e6f4fa; font-family: inherit; font-size: 13.5px; font-weight: 500; cursor: pointer;">
    <span wire:loading.remove wire:target="{{ $selectTarget }}">{{ __('coin.invest.contact_sales') }}</span>
    <span wire:loading wire:target="{{ $selectTarget }}">{{ __('coin.actions.selecting') }}</span>
  </button>
</div>
@else
<div class="coin-plan-card" style="position: relative; padding: 24px; border-radius: 18px; {{ $selectedStyle ?: 'border: 1px solid rgba(150,235,250,0.12); background: rgba(150,235,250,0.04);' }}">
  @if($isSelected)
  <div style="position: absolute; top: -10px; left: 24px; padding: 4px 10px; border-radius: 7px; background: linear-gradient(140deg, oklch(0.88 0.12 192), oklch(0.66 0.13 205)); font-family: 'JetBrains Mono', monospace; font-size: 9px; letter-spacing: 0.12em; color: #04121f;">{{ mb_strtoupper(__('coin.invest.selected')) }}</div>
  @endif
  <div style="display: flex; align-items: center; justify-content: space-between;">
  ...
  <div style="display: flex; flex-direction: column; gap: 13px; font-size: 13px; flex: 1;">
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.min_investment') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedMinDeposit() ?? $plan->formattedComputeLabel() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.term') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedDuration() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.annual_return') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->formattedAnnualProfit() ?? ($plan->formattedDailyEstimate() ?? __('coin.invest.estimated')) }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.infrastructure') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right; color: #f0fbff;">{{ $plan->displayInfra() }}</span></div>
  </div>
  ...
  <button type="button" wire:click="selectPlan({{ $plan->id }})" wire:loading.attr="disabled" wire:target="{{ $selectTarget }}" class="coin-plan-card__cta" style="padding: 11px; border-radius: 10px; border: 1px solid {{ $isSelected ? 'oklch(0.86 0.11 195 / 0.5)' : 'rgba(150,235,250,0.2)' }}; background: {{ $isSelected ? 'linear-gradient(140deg, oklch(0.86 0.12 192), oklch(0.66 0.13 205))' : 'rgba(150,235,250,0.06)' }}; color: {{ $isSelected ? '#04121f' : '#e6f4fa' }}; font-family: inherit; font-size: 13.5px; font-weight: {{ $isSelected ? '600' : '500' }}; cursor: pointer;">
