...
<div style="position: relative; padding: 24px; border-radius: 18px; {{ $currentCardStyle }} display: flex; flex-direction: column;">
  ...
  <div style="display: flex; flex-direction: column; gap: 13px; font-size: 13px;">
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.min_investment') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedMinDeposit() ?? $plan->formattedComputeLabel() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.term') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedDuration() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.annual_return') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedAnnualProfit() ?? ($plan->formattedDailyEstimate() ?? __('coin.invest.estimated')) }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.74); min-width: 0;">{{ __('coin.invest.infrastructure') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->displayInfra() }}</span></div>
  </div>
...
<div style="position: relative; padding: 24px; border-radius: 18px; {{ $selectedStyle ?: 'border: 1px solid rgba(180,180,255,0.16); background: linear-gradient(170deg, rgba(120,110,220,0.13), rgba(150,235,250,0.02));' }} display: flex; flex-direction: column;">
  ...
  <div style="display: flex; flex-direction: column; gap: 13px; font-size: 13px;">
    ...
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.term') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedDuration() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.annual_return') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedAnnualProfit() ?? __('coin.invest.estimated') }}</span></div>
  ...
  <button type="button" wire:click="selectPlan({{ $plan->id }})" style="margin-top: 22px; padding: 11px; border-radius: 10px; border: 1px solid rgba(180,180,255,0.28); background: rgba(150,140,255,0.1); color: #e6f4fa; font-family: inherit; font-size: 13.5px; font-weight: 500; cursor: pointer;">{{ __('coin.invest.contact_sales') }}</button>
</div>
@else
<div style="position: relative; padding: 24px; border-radius: 18px; {{ $selectedStyle ?: 'border: 1px solid rgba(150,235,250,0.12); background: rgba(150,235,250,0.04);' }} display: flex; flex-direction: column;">
  ...
  <div style="display: flex; flex-direction: column; gap: 13px; font-size: 13px;">
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.min_investment') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedMinDeposit() ?? $plan->formattedComputeLabel() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.term') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedDuration() }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.annual_return') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->formattedAnnualProfit() ?? ($plan->formattedDailyEstimate() ?? __('coin.invest.estimated')) }}</span></div>
    <div style="display: flex; justify-content: space-between; gap: 14px;"><span style="color: rgba(214,238,248,0.72); min-width: 0;">{{ __('coin.invest.infrastructure') }}</span><span style="font-family: 'JetBrains Mono', monospace; flex: none; text-align: right;">{{ $plan->displayInfra() }}</span></div>
  </div>
