<x-coin-auth-layout :title="\App\Support\PlatformBrand::pageTitle(__('coin.auth.verify_email_resume_title'))">
    <div class="coin-auth" style="margin: 0 auto; position: relative; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 36px; padding: 80px 72px; box-sizing: border-box; background: #061423; color: #e6f4fa; font-family: 'Sora', 'Helvetica Neue', Helvetica, sans-serif; overflow: hidden;">
        ...
        <div class="coin-auth-card" style="position: relative; padding: 36px 36px 30px; border-radius: 22px; border: 1px solid rgba(150,235,250,0.16); background: linear-gradient(170deg, rgba(13,38,58,0.92), rgba(5,16,27,0.96)); box-shadow: 0 50px 110px -50px #000;">
            <div style="font-size: 22px; font-weight: 600; letter-spacing: -0.025em; color: #f0fbff;">{{ __('coin.auth.verify_email_resume_title') }}</div>
            @include('partials.email-unverified-notice')
            <p style="margin: 14px 0 0; font-size: 14px; line-height: 1.6; color: rgba(230,244,250,0.72);">{{ __('coin.auth.verify_email_resume_sub') }}</p>
            <p style="margin: 10px 0 0; font-size: 13px; line-height: 1.55; color: rgba(230,244,250,0.62);">{{ __('coin.auth.verify_email_login_hint') }}</p>

            <form method="POST" action="{{ route('verification.resume') }}">
                @csrf
                ...
                <button type="submit" style="width: 100%; margin-top: 24px; padding: 15px; border-radius: 12px; border: 1px solid oklch(0.86 0.11 195 / 0.5); background: linear-gradient(140deg, oklch(0.86 0.12 192), oklch(0.66 0.13 205)); color: #04121f; font-family: inherit; font-size: 15px; font-weight: 600; cursor: pointer; box-shadow: 0 20px 46px -22px oklch(0.8 0.13 195 / 0.85);">{{ __('coin.auth.verify_email_resume_action') }}</button>
