<x-coin-auth-layout :title="'Coin — '.__('coin.auth.sign_up')">
            ...
            <form method="POST" action="{{ route('register') }}" x-data="{ visible: false }">
                ...
                <div style="margin-top: 18px;">
                    ...
                    <div style="position: relative; margin-top: 9px;">
                        <input id="password" :type="visible ? 'text' : 'password'" name="password" required autocomplete="new-password" placeholder="{{ __('coin.auth.password_new_placeholder') }}" style="width: 100%; box-sizing: border-box; padding: 14px 92px 14px 16px; border-radius: 12px; border: 1px solid rgba(150,235,250,0.18); background: rgba(4,16,28,0.7); color: #f0fbff; font-family: inherit; font-size: 14.5px;" />
