...
<head>
  ...
  <title>{{ __('coin.auth.verify_email_mail_subject') }}</title>
</head>
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#0b1a28;border:1px solid rgba(150,235,250,0.18);border-radius:16px;overflow:hidden;">
          @include('emails.partials.brand-header', ['title' => __('coin.auth.verify_email_mail_title')])
          <tr>
            <td style="padding:0 28px 18px;font-size:15px;line-height:1.65;color:rgba(214,238,248,0.86);">
              <p style="margin:0 0 14px;">{{ __('coin.auth.verify_email_mail_intro', ['name' => $userName]) }}</p>
              <p style="margin:0;">{{ __('coin.auth.verify_email_mail_body') }}</p>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:0 28px 28px;font-size:13px;line-height:1.6;color:rgba(214,238,248,0.72);">
              <p style="margin:0;">{{ __('coin.auth.verify_email_mail_footer') }}</p>
            </td>
          </tr>
        </table>
      </td>
