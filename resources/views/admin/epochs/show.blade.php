...
@section('content')
    ...
    <div class="admin-card">
        ...
        <p style="margin:8px 0 0;font-size:14px;color:rgba(232,237,245,0.72);">
            {{ $epoch->contracts_settled }} contracts · {{ $epoch->formattedTotalRewards() }} · rate {{ $epoch->reward_rate }} · {{ $epoch->completed_at?->format('M j, Y H:i') }}
        </p>
            ...
            <tbody>
                @forelse($epoch->rewards as $reward)
                    <tr style="border-bottom:1px solid rgba(255,255,255,0.06);">
                        ...
                        <td style="padding:14px 18px;font-family:'JetBrains Mono',monospace;">{{ \App\Support\MoneyFormat::amount($reward->amount) }}</td>
                    </tr>
                @empty
