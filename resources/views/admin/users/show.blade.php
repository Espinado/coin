        ...
        <div>
            <div class="admin-card">
                ...
                <div style="margin-top:14px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;font-size:13px;">
                    <div><span style="color:rgba(232,237,245,0.62);">Registered</span><div style="margin-top:4px;">{{ $user->created_at?->format('M j, Y H:i') ?? '—' }}</div></div>
                    <div><span style="color:rgba(232,237,245,0.62);">Last login</span><div style="margin-top:4px;">{{ $user->last_login_at?->format('M j, Y H:i') ?? '—' }}</div></div>
                    <div><span style="color:rgba(232,237,245,0.62);">Phone</span><div style="margin-top:4px;">{{ $user->phone ?: '—' }}</div></div>
                    <div><span style="color:rgba(232,237,245,0.62);">Telegram</span><div style="margin-top:4px;">{{ $user->telegram ?: '—' }}</div></div>
                    <div><span style="color:rgba(232,237,245,0.62);">Country</span><div style="margin-top:4px;">{{ $user->country_code ?: '—' }}</div></div>
                    <div><span style="color:rgba(232,237,245,0.62);">Referrer</span><div style="margin-top:4px;">@if($user->referrer)<a href="{{ route('admin.users.show', $user->referrer) }}">{{ $user->referrer->accountLabel() }}</a>@else — @endif</div></div>
                </div>
            </div>
            ...
            <div class="admin-card" style="margin-top:16px;">
                ...
                <form method="POST" action="{{ route('admin.users.update', $user) }}" style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    ...
                    <div>
                        <label style="display:block;font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:0.12em;color:rgba(232,237,245,0.65);">PHONE</label>
                    ...
                    <div>
                        <label style="display:block;font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:0.12em;color:rgba(232,237,245,0.65);">TELEGRAM</label>
                    ...
                    <div>
                        <label style="display:block;font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:0.12em;color:rgba(232,237,245,0.65);">COUNTRY CODE</label>
                    ...
                    <div style="grid-column:1/-1;">
                        <label style="display:block;font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:0.12em;color:rgba(232,237,245,0.65);">LEAD NOTE (ADMIN ONLY)</label>
                ...
                @forelse($user->contracts as $contract)
                    <div style="padding:12px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:13px;">
                        ...
                        <div style="margin-top:4px;color:rgba(232,237,245,0.62);">
                            {{ $contract->formattedAnnualProfit() ?? '—' }} APR · {{ $contract->formattedDailyProfit() }}/day · {{ $contract->progress_percent }}% · profit {{ $contract->formattedAccrued() }}
                        </div>
                    </div>
                @empty
            ...
            <div class="admin-card" style="margin-top:16px;">
                ...
                @empty
                    <p style="margin:0;color:rgba(232,237,245,0.62);">No top-ups yet.</p>
                @endforelse
            </div>
